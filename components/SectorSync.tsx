...
const SectorSync: React.FC<SectorSyncProps> = ({ onComplete, onExit, onPlayCorrect, onPlayIncorrect }) => {
  ...
  const { profile, progress, updateProgress } = useFirebase();
  ...
  const handleFinishExam = async () => {
    ...
    if (progress) {
      const report = {
        ...
        percent: Math.round((finalScore / shuffledQuestions.length) * 100),
        durationUsed: 600 - timeRemaining,
      };
      
      const currentReports = progress.assessmentReports || [];
      await updateProgress({
        assessmentReports: [report, ...currentReports].slice(0, 10), // Keep last 10
        lastExamScore: report.percent
      });
    }
  ...
  const readinessPercent = Math.round((currentScore / shuffledQuestions.length) * 100);

  if (!examStarted) {
    return (
      <div className="max-w-4xl mx-auto space-y-12 animate-fade-in text-left py-10 md:py-20 px-6">
  ...
  if (examFinished && !isReviewing) {
    return (
      <div className="max-w-5xl mx-auto py-10 md:py-20 animate-fade-in text-left pb-40 px-6">
            ...
            <div className="space-y-8">
              <div className="p-8 bg-white/[0.03] border border-white/5 rounded-3xl space-y-6">
                <div className="flex justify-between items-center text-[10px] font-black uppercase text-white/30 tracking-widest">
                  ...
                  <span className="text-gold-main">{currentScore} / {shuffledQuestions.length}</span>
                </div>
                ...
                <p className="text-sm text-slate-400 font-light italic leading-relaxed">
                  {readinessPercent >= 70 
                    ? "Resonance achieved. Your acoustic logic is mirroring board expectations." 
                    : "Signal instability detected. Multiple logic errors present."}
                </p>
              </div>
            ...
            <div className="p-10 bg-slate-900 border border-gold-main/20 rounded-[3rem] space-y-8 relative overflow-hidden group">
               ...
               <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] font-black text-gold-main/60 uppercase tracking-[0.4em]">Persistence recorded.</p>
                  </div>
               </div>
            </div>
          </div>
  ...
  return (
    <div className="max-w-6xl mx-auto py-6 md:py-12 animate-fade-in text-left pb-40 px-6">
